...
- 皮肤管理器新增“星光动态特效”开关，可持久化开启或关闭会话背景里的星芒闪烁、光点漂移和亮光呼吸。
- 皮肤管理器顶部增加“星光动态”快捷开关，避免只在详情卡中展示导致用户不易发现。
- Windows 安装器默认选择 `E:\` 作为安装父目录（最终路径 `E:\codex-skin-manager`），无 E 盘时回退到当前用户 Programs 目录。
- Windows 安装器、卸载器和管理器确认/错误弹窗改为统一的梦幻皮肤视觉风格。
- 调整管理器预览框内边距，避免图片遮挡圆角边框；`晨雾珊瑚` 皮肤的背景过渡和 Codex 左栏分隔线改为更柔和的无硬线效果。
- `晨雾珊瑚` 人像皮肤加入 Windows 安装包种子皮肤，重新安装后也会保留为可选皮肤。
